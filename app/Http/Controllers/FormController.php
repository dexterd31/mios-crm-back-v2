...
class FormController extends Controller
{
    ...
    public function report(Request $request, MiosHelper $miosHelper){
        ...
        if ($request->formId == env('SURVEY_FORM')) {
            $formAnswers = $formAnswers->whereNull('form_answers.tipification_time');
        } else {
            $formAnswers = $formAnswers->where('form_answers.tipification_time', '!=', 'upload');
        }

        $formAnswers = $formAnswers->whereBetween('form_answer_logs.updated_at', ["$request->date1 00:00:00", "$request->date2 00:00:00"])
